...
class AuthenticationDelegate(DefaultDelegate):
    ...
    def __init__(self, device):
        ...
        self.device = device
        header = "Time,HRM"
        path = Path(os.path.dirname(os.path.realpath(__file__))) / 'log'
        if not path.exists():
            path.mkdir()
        filename = datetime.datetime.now().strftime("%d_%m_%Y_%H_%M_%S.MiBand2.csv")
        self.f = open(path / filename, 'w')
        #self.f.write(header + '\n') 
    ...
    def handleNotification(self, hnd, data):
        ...
        if hnd == self.device.char_auth.getHandle():
            ...
            heart = str(rate)
            time = datetime.datetime.now().strftime("20%y-%m-%d %H:%M:%S")
            print("Time: " + time + ", Heart Rate: " + heart)
            self.f.write(time + "," + heart + "\n")
...
def main():
    ...
    pathToKey = Path(os.path.dirname(os.path.realpath(__file__))) / 'mac_key.txt'

    f = open(pathToKey.resolve(), 'r')
    mac_key = None  
    for line in f:
        mac_key = line    
        break
    ...
    print()
    print("Results are saved in file: " + band.delegate.f.name)    
